...
Advanced web design agents are for planning and review only. Codex is suitable for documentation-only design agent changes in this repository. Actual design asset creation, Figma work, screenshot generation, prototype creation, user research execution, survey creation, transcript handling, component implementation, package installation, app routes, style files, and runtime configuration are out of scope for this repository unless a future scope explicitly allows them. This GPT project should not directly modify GitHub or create design assets.
