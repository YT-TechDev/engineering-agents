...
AI Application / Agent Workflow agents are for planning and review only. AI application boundary, prompt/context boundary, RAG/retrieval boundary, AI evaluation, agent tool permission, model cost/latency, AI SDK/dependency, model provider vendor, MCP server boundary, plugin/extension supply chain, AI data processor boundary, and AI vendor risk review should remain documentation-only in this repository. Codex is suitable for small documentation-only AI vendor/supply-chain agent changes, while GPT remains the coordinator for planning, prompt creation, review, and merge-readiness judgment. This repository should not create AI SDK integrations, dependency updates, package files, lockfiles, SBOMs, provider setup guides, MCP configs, plugin manifests, extension configs, legal templates, compliance docs, generated reports, releases, tags, automation, or implementation artifacts unless a future scope explicitly allows them. GitHub write operations from this GPT project remain out of scope. Vendor, processor, provider, plugin, extension, and MCP decisions must remain human-owned and specialist-reviewed when they affect security, privacy, legal/procurement, release, cost, or user trust.
